...
# Tab 3
st.set_page_config(page_title="Tab 3: Match Analysis")
...
st.text("Please upload the events & points csv files in order.")
uploaded_file = st.file_uploader("Please choose your files:", accept_multiple_files = True)

if len(uploaded_file) == 2:
    
    df3 = pd.read_csv(uploaded_file[0])
    
    data1 = df3.copy()
    data1.drop(["receiver_x","receiver_y","receiver"], axis=1, inplace=True)
    data1["type"] = "hit position"
    data1.rename(columns={'hitter_x':'x', 'hitter_y':'y','hitter':'player'}, inplace=True)
    
    data2 = df3.copy()
    data2.drop(["hitter_x", "hitter_y","hitter"], axis=1, inplace=True)
    data2["type"] = "receive position"
    data2.rename(columns={'receiver_x':'x', 'receiver_y':'y','receiver':'player'}, inplace=True)
    
    df_new = pd.concat([data1,data2], axis = 0, join = 'outer', ignore_index = False) 
...
#data = pd.read_csv(path + filename)
    ...
    df4 = pd.read_csv(uploaded_file[1])
    
    players2 = df4["winner"].unique()
    player1 = players2[0]
    player2 = players2[1]
    ...
    if option4 == 'Select all':
        ...
        data_option = df4[df4["reason"]==option4]
    ...
    win_reasons = df4.groupby("reason")["winner"].value_counts().unstack()
